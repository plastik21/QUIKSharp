...
namespace QuikSharp
{
    public sealed class QuikService
    {
        ...
        static QuikService()
        {
            Services = new Dictionary<int, QuikService>();
        ...
        private readonly AsyncManualResetEvent _connectedMre;
        private readonly Channel<IMessage> _receivedCallbacksChannel;
        ...
        /// </summary>
        internal readonly ConcurrentDictionary<long, KeyValuePair<TaskCompletionSource<IMessage>, Type>> Responses;
        ...
        private QuikService(int responsePort, string host)
        {
            ...
            _connectedMre = new AsyncManualResetEvent();

            _receivedCallbacksChannel = Channel.CreateUnbounded<IMessage>(new UnboundedChannelOptions()
            {
                SingleReader = true,
                SingleWriter = true
            });

            SessionId = DateTime.Now.ToString("yyMMddHHmmss");

            Events = new QuikEvents(this);
            EnvelopeQueue = new BlockingCollection<IMessage>();
                                ...
                                while (!_cts.IsCancellationRequested)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            Responses[message.Id.Value].Key.SetException(new TimeoutException(
                                                "ValidUntilUTC is less than current time"));

                                            Responses.TryRemove(message.Id.Value, out KeyValuePair<TaskCompletionSource<IMessage>, Type> tcs);
                                        }
                                    }
            ...
            _callbackReceiverTask = Task.Factory.StartNew(async () =>
            {
                try
                {
                    ...
                    while (!_cts.IsCancellationRequested)
                    {
                        ...
                        // Оповещаем клиента что произошло подключение к терминалу
                        ...
                        catch (IOException e)
                        {
                            ...
                            // Оповещаем клиента что произошло отключение от терминала
        ...
        public void Stop()
        {
            ...
            try
            {
                ...
                var isCleanExit = Task.WaitAll(new[]
                {
                    _requestTask,
                    _responseTask,
                    _callbackReceiverTask,
                    _callbackInvokerTask
                },
                5000);
        ...
        {
            return
                _responseClient != null && _responseClient.Connected && _responseClient.Client.IsConnectedNow() &&
        ...
        private void EnsureConnectedClient(CancellationToken ct)
        {
            lock (_syncRoot)
            {
                // Ensure responseClient is connected
                if (_responseClient == null || !_responseClient.Connected || !_responseClient.Client.IsConnectedNow())
                {
                    ...
                    while (!connected)
                    {
                        ...
                        catch
                        {
                            ...
                            if (++attempt % 10 == 0)
                            {
                                Trace.WriteLine($"Trying to connect... {attempt}");
                            }
                        }
                    }
                }

                // Ensure callbackClient is connected
        ...
        internal int GetUniqueTransactionId()
        {
            ...
            {
                Console.WriteLine("Неудачная попытка записи нового ID в файл MMF: " + er.Message);
            }
            ...
            where TResponse : class, IMessage, new()
        {
            ...
            if (timeout > 0)
            {
                ...
                ctRegistration = ct.Token.Register(() =>
                {
                    ...
                    Responses.TryRemove(request.Id.Value, out KeyValuePair<TaskCompletionSource<IMessage>, Type> temp);
                },
                false);
